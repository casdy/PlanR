...
import { applyDeloadAdjustment } from '../engine/deloadEngine';
...
export const RoutineGenerator = ({ onRoutineGenerated }: { onRoutineGenerated: (program: Omit<WorkoutProgram, 'id' | 'userId' | 'version'>) => void }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const status = await checkDeloadStatus();

            if (result.type === 'program') {
                // ExerciseDB fallback path — no streaming
                setUsedFallback(true);
                let finalProg = result.program;
                if (status.isDeloadRecommended) {
                    finalProg.schedule = finalProg.schedule.map(applyDeloadAdjustment);
                }
                setTimeout(() => onRoutineGenerated(finalProg), 800);
            } else {
                // Groq streaming path
                let fullText = '';
                ...
                if (match) {
                    let program = JSON.parse(match[0]);
                    if (status.isDeloadRecommended) {
                        program.schedule = program.schedule.map(applyDeloadAdjustment);
                    }
        ...
        <Card className="p-6 bg-gradient-to-br from-primary/5 to-card dark:from-primary/10 dark:to-card border-primary/10 overflow-hidden relative rounded-[2.5rem]">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-black tracking-tight">{t('ai_workout_architect')}</h2>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <div className={cn(
                        ...
                        {quotaExhausted ? t('planr_engine_label') : t('prompts_left', { remaining })}
                    </div>
                </div>
            </div>
            ...
            {quotaExhausted && (
                <div className="flex items-start gap-3 p-3 rounded-2xl bg-orange-500/5 border border-orange-500/20 mb-4">
                    ...
                    <p className="text-xs text-muted-foreground font-medium">
                        {t('quota_reached_msg')}
                        <div className="mt-1 font-black text-[10px] uppercase text-orange-400">
                             {quotaService.getResetMessage()}
                        </div>
                    </p>
                </div>
            )}
            ...
            <textarea
                ...
                placeholder={t('routine_goal_placeholder')}
            ...
            >
                ...
                {isGenerating
                    ? (usedFallback ? t('building_from_library') : t('architecting_routine'))
                    : (quotaExhausted ? t('generate_via_planr') : t('generate_new_routine'))
                }
            </Button>
                ...
                {isGenerating && !usedFallback && (
                    ...
                    >
                        <div className="flex items-center justify-between px-1">
                            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">{t('progressive_discovery')}</span>
                            <span className="text-[11px] font-mono text-primary">{t('exercises_found', { count: discoveredExercises.length })}</span>
                        </div>
